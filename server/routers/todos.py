...
router = APIRouter(
    prefix="/todos", tags=["todos"], responses={404: {"user": "Not Found"}}
)
...
async def read_all_by_user(request: Request, db: Session = Depends(get_db)):
    ...
    todos = db.query(model.Todos).filter(model.Todos.user_id == user.get("id")).all()
...
async def edit_todo_commit(
    ...
    db.add(todo_model)
    db.commit()
    return RedirectResponse(url=f"/todos", status_code=status.HTTP_302_FOUND)
...
async def delete_todo(request: Request, todo_id: int, db: Session = Depends(get_db)):
    ...
    todo_model = (
        db.query(model.Todos)
        .filter(model.Todos.id == todo_id)
        .filter(model.Todos.user_id == user.get("id"))
        .first()
    )
...
async def read_todo_by_id(
    user_id: int, user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail={"user not found"}
    ...
    if todo_model is not None:
        ...
        return JSONResponse(
            content=jsonable_encoder({"Message": "Item Not Found"}), status_code=404
        )
...
async def delete_todo(
    ...
    db.commit()
    return JSONResponse(
        content=jsonable_encoder({"Message": "Item deleted successfully"}),
